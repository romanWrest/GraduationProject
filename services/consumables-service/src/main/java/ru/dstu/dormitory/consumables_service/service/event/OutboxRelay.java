...
import ru.dstu.dormitory.consumables_service.config.KafkaTopicsProperties;
import ru.dstu.dormitory.consumables_service.config.OutboxProperties;
...
public class OutboxRelay {
    ...
    private void publish(OutboxEvent event) {
        try {
            ...
            kafkaTemplate.send(topic, event.getAggregateId(), wrap(event, payloadNode)).get();
    ...
    /**
     * События для notification-service ({@code ConsumableReturnRequired}) идут в общий
     * notification-events topic. Все остальные — в consumable.events.
     */
    private String resolveTopic(String eventType) {
        if (EventTypes.CONSUMABLE_RETURN_REQUIRED.equals(eventType)) {
            return topics.getNotificationEvents();
        }
        return topics.getConsumableEvents();
    }
