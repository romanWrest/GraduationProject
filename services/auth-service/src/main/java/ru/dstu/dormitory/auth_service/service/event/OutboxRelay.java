...
import ru.dstu.dormitory.auth_service.config.KafkaTopicsProperties;
import ru.dstu.dormitory.auth_service.config.OutboxProperties;
import ru.dstu.dormitory.auth_service.domain.model.OutboxEvent;
import ru.dstu.dormitory.auth_service.domain.repo.OutboxEventRepository;
...
public class OutboxRelay {
    ...
    @Transactional
    public void relay() {
        int batch = outboxProperties.getRelay().getBatchSize();
    ...
    private void publish(OutboxEvent event) {
        try {
            ...
            kafkaTemplate.send(topics.getUserEvents(), event.getAggregateId(), wrap(event, payloadNode)).get();
