package ru.dstu.dormitory.reports_service.web.advice;
...
import ru.dstu.dormitory.reports_service.dto.error.ErrorResponseDTO;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponseDTO> handleReportNotFound(ReportNotFoundException ex) {
        return setErrorResponseDTO(ex, "Отчёт не найден", HttpStatus.NOT_FOUND);
    }
    ...
    public ResponseEntity<ErrorResponseDTO> handleInvalidPeriod(InvalidReportPeriodException ex) {
        return setErrorResponseDTO(ex, "Некорректный период отчёта", HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(UnsupportedExportFormatException.class)
    public ResponseEntity<ErrorResponseDTO> handleUnsupportedFormat(UnsupportedExportFormatException ex) {
        return setErrorResponseDTO(ex, "Неподдерживаемый формат экспорта", HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(ReportRowLimitExceededException.class)
    public ResponseEntity<ErrorResponseDTO> handleRowLimit(ReportRowLimitExceededException ex) {
        return setErrorResponseDTO(ex, "Превышен лимит строк отчёта", HttpStatus.BAD_REQUEST);
    }

    // ── 500 ──

    @ExceptionHandler(ExportRenderException.class)
    public ResponseEntity<ErrorResponseDTO> handleExportRender(ExportRenderException ex) {
        return setErrorResponseDTO(ex, "Ошибка генерации экспорта", HttpStatus.INTERNAL_SERVER_ERROR);
    }
